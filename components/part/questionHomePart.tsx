...
const QuestionPartComponent =async () => {
    ...
    return (
       data.map((item:any,index:number)=>(
           <div key={index}>
               <div className="border h-[240px] border-gray-500/20 rounded-md shadow-[rgb(31_45_61_/_10%)_0px_2px_10px_1px] dark:shadow-[0_2px_11px_0_rgb(6_8_24_/_39%)] p-6 pt-12 mt-8 relative">
                   ...
                   <Link href={`/question/detail/${item.currentSlug}`}>
                       <h5 className="text-dark leading-7 text-lg font-semibold mb-3.5 dark:text-white-light h-[50px] ">
                           <span className="line-clamp-2">{item.title}</span>
                       </h5>
                   </Link>
                   <Link href={`/question/detail/${item.currentSlug}`}>
                       <p className="text-white-dark text-[15px]  leading-6 mb-3.5 line-clamp-3">
